...
l(a,$(this).find("option:selected")[0].value)})}var k=$(".contribution-requests");if(-1!==r.indexOf("contribution_requests")){var P=/contribution_requests\/([0-9]+)/.exec(window.location)[1],N=$(".o-c-r"),O=$(".c-c-r");N.on("click",function(a){N.addClass("active");O.removeClass("active");$.ajax({type:"get",url:"/get_open_contribution_requests",datatype:"json",data:{tale_id:P},success:function(a){k.find(".closed-contribution-requests").remove();k.append(a)},error:function(a,b,d){console.log(a)}})});
N.trigger("click");O.on("click",function(a){O.addClass("active");N.removeClass("active");$.ajax({type:"get",url:"/get_closed_contribution_requests",datatype:"json",data:{tale_id:P},success:function(a){k.find(".open-contribution-requests").remove();k.append(a)},error:function(a,b,d){console.log(a)}})})}if(-1!==r.indexOf("create")){var Q=$(".create form");Q.form({title:{identifier:"create-title",rules:p.title},description:{identifier:"create-description",rules:p.description},genres:{identifier:"create-genres",
rules:[{type:"empty_genres",prompt:n[m].EMPTY}]},license:{identifier:"create-license",rules:p.checked},type:{identifier:"create-type",rules:p.checked}},{rules:{empty_genres:function(){return 0<$('[name="create-genres"]')[0].children.length}},inline:!0,onSuccess:function(){$.ajax({type:this.method,url:this.action,data:new FormData(this),processData:!1,contentType:!1,success:function(a){window.location=a.url},error:function(a,b,d){v(a.responseJSON.error_list,Q.find("h1"))}});return!1}});var q=$("#genres"),
K=$('select[name="create-genres"]'),f=$('select[name="available-genres"]');$("#available-genres").on("keyup",function(a){a=this.value;for(var b=f[0],d=0;d<b.length;d++)0>b[d].innerHTML.indexOf(a)?$(b[d]).attr("hidden","true"):$(b[d]).removeAttr("hidden")});$("#taken-genres").on("keyup",function(a){a=this.value;for(var b=K[0],d=0;d<b.length;d++)0>b[d].innerHTML.indexOf(a)?$(b[d]).attr("hidden","true"):$(b[d]).removeAttr("hidden")});$("#add").on("click",function(a){a=f.find("option:selected");for(var b=
0;b<a.length;b++)K.append(a[b])});$("#remove").on("click",function(a){a=K.find("option:selected");for(var b=0;b<a.length;b++)f.append(a[b])});q.on("dblclick",'select[name="create-genres"] option',function(a){f.append(this);K.find("option").each(function(){$(this).attr("selected","selected")})});q.on("dblclick",'select[name="available-genres"] option',function(a){K.append(this)})}if(-1!==r.indexOf("contribute")){var g=$(".contribute-form");tinymce.init({selector:'textarea[name="contribute-content"]'});
g.form({title:{identifier:"contribute-title",rules:p.title}},{inline:!0,onSuccess:function(){$.ajax({type:this.method,url:this.action,data:{"contribute-title":g.find('[name="contribute-title"]')[0].value,"contribute-content":tinymce.get("contribute-content").getContent(),_csrf_token:g.find('[name="_csrf_token"]')[0].value},success:function(a){window.location=a.url},error:function(a,b,d){v(a.responseJSON.error_list,g.find("h1"))}});return!1}})}if(-1!==r.indexOf("settings"))$("#form-delete").on("submit",
function(a){a.preventDefault();a=$(this).find("button")[0].value;var b=prompt(n[m].DELETE_MODAL_MESSAGE);b&&(b===a?$.ajax({type:this.method,url:this.action,datatype:"json",data:new FormData(this),processData:!1,contentType:!1,success:function(a){window.location=a.url},error:function(a,b,c){console.log(a)}}):($(this).find("div").remove(),$(this).append('<div class="ui red message">'+n[m].DELETE_MODAL_WRONG_TITLE+"</div>")));return!1})});"use strict";
...
e,f,l){var k=null,g=h.slice(),a=d,c;3===arguments.length?"number"!==typeof f&&(l=f,f=1):2===arguments.length&&(f=1);if(f!==parseInt(f,10)||1>f)throw"numRounds must a integer >= 1";switch(e){case "HEX":k=A;break;case "B64":k=y;break;case "BYTES":k=z;break;default:throw"format must be HEX, B64, or BYTES";}if("SHA-224"===b)for(c=0;c<f;c+=1)g=q(g,a,b),a=224;else if("SHA-256"===b)for(c=0;c<f;c+=1)g=q(g,a,b),a=256;else throw"Chosen SHA variant is not supported";return k(g,C(l))};this.getHMAC=function(b,
